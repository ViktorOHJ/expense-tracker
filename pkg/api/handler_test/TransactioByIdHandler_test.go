...
func TestTransactionByIdHandler_Success(t *testing.T) {
	...
	mockDB.On("GetTransactionByID", mock.Anything, 1).Return(expectedTransaction, nil)
	...
	// Данные внутри SuccessResponse.Data — это interface{}, надо преобразовать в нужный тип
	// Для этого сериализуем обратно в JSON, затем распарсим в Transaction (грубое, но простое решение)
...
func TestTransactionByIdHandler_NotFound(t *testing.T) {
	mockDB := new(mocks.DB)
	s := api.NewServer(mockDB)
	mockDB.On("GetTransactionByID", mock.Anything, 1).Return(models.Transaction{}, db.ErrNotFound)
	...
	assert.Equal(t, "transaction with id 1 not found", actualResp.Message)
